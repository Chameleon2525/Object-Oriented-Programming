...
#ifndef _EasyBMP_Geometry_h_
#define _EasyBMP_Geometry_h_
...
void DrawAALine( BMP &Image , int FromX, int FromY, int ToX, int ToY , RGBApixel Color )
{
 ...
 for( i=LeftI ; i <= RightI ; i++ )
 {
  for( j=LeftJ ; j <= RightJ ; j++ )
  {
   ...
   if( Temp > MinValue )
   {
    Image(i,j)->Red = (BYTE) (unsigned int) (  (1.0-Temp)*( (double) Image(i,j)->Red )
                                                        +Temp*( (double) Color.Red ) );
    Image(i,j)->Green = (BYTE) (unsigned int) (  (1.0-Temp)*( (double) Image(i,j)->Green )
                                                        +Temp*( (double) Color.Green ) );
    Image(i,j)->Blue = (BYTE) (unsigned int) (  (1.0-Temp)*( (double) Image(i,j)->Blue )
              ...
              double FromTheta, double ToTheta , RGBApixel Color )
{
 ...
 for( i=0 ; i < Downsampled.TellWidth() ; i++ )
 {
  for( j=0 ; j < Downsampled.TellHeight() ; j++ )
  {
   ...
   Downsampled(i,j)->Red = (BYTE) TempRed;
  }
 }
 
 // paste with alpha blending 
 
 int DestinationTopLeft = Round( CenterX - Radius );
 int DestinationTopRight = Round( CenterY - Radius );
 ...
 for( i=0 ; i < Downsampled.TellWidth() ; i++) 
 {
  for( j=0 ; j < Downsampled.TellHeight() ; j++)
  {
       ...
       DestinationJ >= 0 && DestinationJ < Image.TellHeight() )
   {
    ...
    Image(DestinationI,DestinationJ)->Red = (BYTE) ( 
      (1.0-alpha)*Image(DestinationI,DestinationJ)->Red 
    + (alpha)*Color.Red );
    Image(DestinationI,DestinationJ)->Green = (BYTE) ( 
      (1.0-alpha)*Image(DestinationI,DestinationJ)->Green 
    + (alpha)*Color.Green );
    Image(DestinationI,DestinationJ)->Blue = (BYTE) ( 
